...
dynamic routes all render in Tor Browser. It ships two ways: as a library (`start using`) and as one
paste-and-run `examples/onion-httpd/standalone.livecodescript`, generated by `tools/build-standalone.py`
(which concatenates the two libraries + the demo and refuses to build on a name collision).
